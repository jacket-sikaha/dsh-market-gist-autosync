...
*/
function gistHttp(token, method, path, body, host = "api.github.com", signal) {
	return new Promise((resolve) => {
		...
		const req = request({
			...
			const chunks = [];
			let size = 0;
			const maxBytes = GIST_MAX_BYTES + 16384;
			const declared = Number(res.headers["content-length"]);
			if (Number.isFinite(declared) && declared > maxBytes) {
				res.destroy(/* @__PURE__ */ new Error("GitHub response is too large"));
				return;
			}
			res.on("data", (chunk) => {
				size += chunk.length;
				chunks.push(chunk);
				if (size > maxBytes) res.destroy(/* @__PURE__ */ new Error("GitHub response is too large"));
			});
			res.on("end", () => resolve({
		...
		req.on("error", (e) => resolve({
			...
			body: "",
			netError: e.message,
			netErrorCode: classifyNetError(e)
		}));
...
async function createGist(token, content, host, signal) {
...
async function updateGist(token, gistId, content, host, signal) {
...
async function verifyToken(token, host, signal) {
	const r = await gistHttp(token, "GET", "/user", void 0, host, signal);
	if (r.netError) return failNet(r);
	if (r.status !== 200) return classify(r.status, r.body);
	return {
		ok: true,
...
async function readGistBackupContent(token, gistId, host, signal) {
	const r = await gistHttp(token, "GET", `/gists/${gistId}`, void 0, host, signal);
	if (r.netError) return failNet(r);
	if (r.status !== 200) return classify(r.status, r.body);
	let data;
	try {
		...
		return err("invalid", "Gist 响应不是有效 JSON");
	}
	...
	if (typeof candidate !== "string") return err("invalid", "Gist 中找不到 dsh 备份文件");
	return {
		ok: true,
		content: candidate
	};
...
*/
function stripMachineLocalDeps(manifest) {
	const json = manifest !== null && typeof manifest === "object" && !Array.isArray(manifest) ? manifest : {};
	...
	const dependencies = {};
	for (const [name, spec] of Object.entries(json.dependencies)) if (!drop.has(name)) dependencies[name] = spec;
	const dsh = json.dsh !== null && typeof json.dsh === "object" && !Array.isArray(json.dsh) ? json.dsh : {};
	const profile = dsh.profile !== null && typeof dsh.profile === "object" && !Array.isArray(dsh.profile) ? dsh.profile : {};
	const declared = Array.isArray(profile.bundles) ? profile.bundles : [];
	const strippedBundles = declared.filter((n) => typeof n === "string" && drop.has(n));
	return {
		json: {
			...json,
			dependencies,
			dsh: {
				...
				profile: {
					...profile,
					bundles: declared.filter((n) => typeof n !== "string" || !drop.has(n))
				}
			}
		},
		strippedDeps,
...
async function installRestoredDeps(root, onProgress) {
	...
	const prunedDead = [];
	const failedPrune = [];
	for (const name of deadLocal) if (pruneDependency(manifestFile, name)) prunedDead.push(name);
	else failedPrune.push(name);
	if (failedPrune.length > 0) return {
		ok: false,
		installed: false,
	...
	if (missing.length === 0 && prunedDead.length === 0) return {
		ok: true,
		installed: true,
		summary: "依赖均已安装",
		installedNames: [],
	...
	if (install.spawnError) {
		const removedNote = prunedDead.length > 0 ? `另有 ${prunedDead.length} 个指向本机不存在路径的本地依赖已从 profile 移除：${prunedDead.join("、")}。` : "";
		return {
			ok: true,
			installed: false,
	...
	const stuckNote = stuck.length > 0 ? `；${stuck.length} 个依赖装不上且无法从 package.json 移除（文件可能不可写）：${stuck.join("、")}` : "";
	if (failed.length === 0 && stuck.length === 0) return {
		ok: true,
		installed: true,
	...
	return {
		ok: true,
		installed: false,
		summary: failed.length > 0 ? `依赖安装失败，已剔除装不上的：${failed.join("、")}。请在插件市场手动重装。${stuckNote}` : `依赖安装失败${stuckNote}。请在插件市场手动重装。`,
		installedNames,
		prunedNames: failed
	};
}
...
*/
function parsePatchList(text, dshInstallDir) {
	let yaml;
	const anchors = [fileURLToPath(import.meta.url)];
	...
	try {
		...
		for (const entry of parsed) if (entry === null || typeof entry !== "object" || Array.isArray(entry)) return null;
		return parsed;
	} catch {
		return null;
	}
...
function analyzeBundles(profileDirectory, dshInstallDir = findDshInstallDir()) {
	...
	return names.map((name) => {
		...
		for (const anchor of anchors) {
			...
			if (typeof directory === "string") return judgeResolvedBundle(name, directory, dshInstallDir);
...
*/
function judgeResolvedBundle(name, directory, dshInstallDir) {
	const resolved = {
		name,
		directory,
		error: null,
	...
	let declared;
	try {
		declared = JSON.parse(readFileSync(join(directory, "package.json"), "utf8"))?.dsh?.bundle?.patch;
	} catch {
		return {
			...resolved,
			error: null,
	...
	const declaredList = typeof declared === "string" ? [declared] : Array.isArray(declared) ? declared.filter((item) => typeof item === "string") : [];
	if (declaredList.length === 0 || declaredList.some((rel) => rel === "")) return {
		...resolved,
		error: "bundle declares no dsh.bundle.patch - the profile will fail to boot"
	};
	const texts = [];
	for (const relative of declaredList) try {
...
function orphanBundles(profileDirectory, dshInstallDir = findDshInstallDir()) {
	try {
		return analyzeBundles(profileDirectory, dshInstallDir).filter((layer) => layer.error !== null && !layer.unresolvedInbox).map((layer) => layer.name);
	} catch {
		return [];
	}
}
...
function readManifestDeps(profileDirectory) {
	try {
		return JSON.parse(readFileSync(resolve(profileDirectory, "package.json"), "utf8"))?.dependencies;
	} catch {
		return;
	}
}
...
async function doBackup(cfg, host, gistOverride) {
	...
	const stripNote = strippedDeps.length > 0 ? `；已排除 ${strippedDeps.length} 个指向本机路径的本地依赖（换机器后路径不存在，同步过去也无法安装）：${strippedDeps.map((d) => `${d.name}（${d.spec}）`).join("、")}${strippedBundles.length > 0 ? `，并同时移除了对应的 bundle 记录：${strippedBundles.join("、")}` : ""}` : "";
	return {
		ok: true,
		gistId: newGistId,
		gistUrl,
		...
		strippedDeps,
		strippedBundles,
		message: stripNote === "" ? void 0 : `备份完成${stripNote}`
	};
}
async function doRestore(cfg, host, gistInput, onProgress) {
	...
	const warnNote = warnings.length > 0 ? `；另有 ${warnings.length} 个依赖指向本机绝对路径，${warnings.map((w) => `${w.name}（${w.spec}）`).join("、")}——本机可用，但换机器后路径不存在，建议改为可移植的来源或在本机插件市场重装` : "";
		...
		handler: async (request, response) => {
			...
			try {
				...
				if (action === "getConfig") {
					...
					if (restoreProgress.active) sendJson(response, 409, {
						ok: false,
