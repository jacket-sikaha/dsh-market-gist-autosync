...
import { deviceName, activeProfile, readBackupConfig, writeBackupConfig, scheduleIntervalMs, GIST_TOKEN_ENV, } from './config.js';
...
let restoreProgress = { active: false, lines: [], done: true };
            ...
            try {
                ...
                if (action === 'getConfig') {
                    ...
                    const result = await doRestore(readBackupConfig(), apiHost, gistInput, (p) => {
                        ...
                        if (line)
                            restoreProgress.lines.push(line);
                    });
