...
async function installRestoredDeps(root, onProgress) {
	...
	const missing = deps.filter(([name, spec]) => {
		...
		if (/^(?:link|file):/.test(spec)) return false;
...
async function doBackup(cfg, host) {
	...
	const record = {
		...
		gistUrl,
		bytes,
		createdAt: String(ref.createdAt || (/* @__PURE__ */ new Date()).toISOString()),
		updatedAt: String(ref.updatedAt || (/* @__PURE__ */ new Date()).toISOString())
	};
	writeBackupConfig({
		...cfg,
		gistId: newGistId,
	...
	const schedule = (cfg) => {
		...
		if (cfg.scheduleEnabled) {
			interval = setInterval(() => {
				doBackup(readBackupConfig(), apiHost).then((r) => {
	...
	ctx.webServer.register({
		...
		handler: async (request, response) => {
			...
			try {
				...
				if (action === "getConfig") {
				...
				else if (action === "backupNow") sendJson(response, 200, await doBackup(readBackupConfig(), apiHost));
					...
					ok: true,
					uploads: readBackupConfig().uploads || []
				});
...
//#endregion
export { Config, apply, collectProfileBackup, inject, installRestoredDeps, mergeManifests, name, restoreBackup, serializeBackup, validateBackupStrict };
