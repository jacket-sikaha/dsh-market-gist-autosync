...
function isConfigFile(n) {
    return (n === 'settings.yaml' ||
        n === 'dream-skin.json' ||
...
async function doTest(cfg, host) {
    if (!cfg.gistToken.trim())
        return err('no_token', '未配置 Gist token');
