...
*/
var uploadRecordSchema = z.object({
	gistId: z.string(),
	deviceName: z.string(),
	uploadedAt: z.string(),
...
async function apply(ctx, rawConfig) {
	...
	let interval;
	const storePromise = openUploadStore(ctx).then(async (store) => {
		if (store) {
	...
	ctx.webServer.register({
		...
		handler: async (request, response) => {
			...
			try {
				...
				else if (action === "backupNow") sendJson(response, 200, await runBackup());
...
//#endregion
export { Config, apply, collectProfileBackup, inject, installRestoredDeps, mergeManifests, migrateLegacyUploads, name, openUploadStore, restoreBackup, serializeBackup, uploadDomainSpec, validateBackupStrict };
