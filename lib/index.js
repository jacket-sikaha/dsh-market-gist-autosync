...
import { existsSync, readFileSync, readdirSync, mkdirSync, writeFileSync, lstatSync, renameSync, rmSync, } from 'node:fs';
...
async function readGistBackup(token, gistId, host) {
    ...
    if (typeof candidate !== 'string')
        return err('invalid_gist', 'Gist 中找不到 dsh 备份文件');
    let parsed;
    try {
 ...
 */
async function installRestoredDeps(root) {
    ...
    if (deps.length === 0)
        return { ok: true, installed: true, summary: '无依赖需要安装' };
    ...
    if (missing.length === 0)
        return { ok: true, installed: true, summary: '依赖均已安装' };
    ...
    if (failed.length === 0)
        return { ok: true, installed: true, summary: `依赖安装完成（新装 ${installedCount} 个）` };
    if (installedCount > 0) {
