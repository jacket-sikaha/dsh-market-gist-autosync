 ...
 */
import { GIST_MAX_BYTES, activeProfile, err, parseGistId, resolveToken, writeBackupConfig, profileRoot, MAX_UPLOAD_RECORDS, } from './config.js';
import { createGist, updateGist, verifyToken, readGistBackupContent, gistHttp, classify } from './gist.js';
import { collectProfileBackup, serializeBackup, buildBackupEnvelope, validateBackupStrict } from './backup.js';
import { restoreBackup } from './restore.js';
import { installRestoredDeps } from './install.js';
export async function doTest(cfg, host) {
...
export async function doBackup(cfg, host) {
    const resolved = resolveToken(cfg);
    if (!resolved)
        return err('no_token', '未配置 Gist token（请在下方填写，或设置环境变量 DSH_GITHUB_TOKEN）');
    let gid;
...
export async function doRestore(cfg, host, gistInput, onProgress) {
    const resolved = resolveToken(cfg);
    if (!resolved)
        return err('no_token', '未配置 Gist token（请在下方填写，或设置环境变量 DSH_GITHUB_TOKEN）');
    let gid;
    ...
    if (!result.ok)
        return err('restore_failed', result.error || '恢复失败');
