...
import { GIST_FILENAME, GIST_MAX_BYTES, REQUEST_TIMEOUT_MS, err } from './config.js';
export function gistHttp(token, method, path, body, host = 'api.github.com') {
...
export async function createGist(token, content, host) {
...
export async function updateGist(token, gistId, content, host) {
    const body = JSON.stringify({
        files: { [GIST_FILENAME]: { content } },
    });
...
export async function verifyToken(token, host) {
...
export async function readGistBackupContent(token, gistId, host) {
