...
// ---------------------------------------------------------------------------
// Post-restore dependency install. Restoring only edits package.json; without
// a `pnpm install` the merged manifest can reference packages that are not in
// node_modules, and the next boot fails to resolve those bundles -> recovery
// mode. This mirrors dshmarket's restore: install, and on failure retry each
// dependency individually, pruning the ones that cannot install so the profile
// always boots.
// ---------------------------------------------------------------------------
...
function spawnCmd(file: string, args: string[], cwd: string): Promise<PnpmResult> {
...
function pruneDependency(manifestFile: string, name: string): void {
  try {
...
/**
 * Install the dependencies the restored manifest now references, so the next
 * boot can resolve every bundle (otherwise -> recovery mode).
 *
 ...
 *   plugins that are already present. We report that a manual reinstall check
 *   is needed instead.
 ...
 */
async function installRestoredDeps(root: string): Promise<{ ok: boolean; installed: boolean; summary: string }> {
  ...
  try {
    ...
    return { ok: false, installed: false, summary: '无法读取恢复后的 package.json' }
  }
  const deps = Object.entries(manifest.dependencies ?? {})
  if (deps.length === 0) return { ok: true, installed: true, summary: '无依赖需要安装' }
  ...
  if (missing.length === 0) return { ok: true, installed: true, summary: '依赖均已安装' }
  ...
  if (install.exitCode === 0) {
    return { ok: true, installed: true, summary: `依赖安装完成（新装 ${missing.length} 个）` }
  }
  ...
  for (const [name, spec] of missing) {
    ...
    if (present) {
      ...
      return { ok: true, installed: false, summary: `安装中断（${r.spawnError}）。请在重启后于插件市场确认依赖。` }
    } else {
      failed.push(name)
      pruneDependency(manifestFile, name)
    }
  }
  if (failed.length === 0) return { ok: true, installed: true, summary: `依赖安装完成（新装 ${installedCount} 个）` }
  if (installedCount > 0) {
    return { ok: true, installed: true, summary: `已装 ${installedCount} 个，剔除装不上的：${failed.join('、')}（可在插件市场重装）` }
  }
  return { ok: true, installed: false, summary: `依赖安装失败，已剔除装不上的：${failed.join('、')}。请在插件市场手动重装。` }
}
...
async function doRestore(cfg: GistBackupConfig, host: string, gistInput: string): Promise<Result> {
  ...
  // resolve every bundle (otherwise -> recovery mode). Un-installable deps are
  // pruned from the manifest so the profile always boots.
  const install = await installRestoredDeps(profileRoot())
  const installNote = install.ok ? install.summary : `${install.summary}`
  return {
    ok: true,
